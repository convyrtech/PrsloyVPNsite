  ...
  en: {
    ...
    entries: [
      ...
      },
      {
        ...
        body:
          "Beta access is now fully self-serve. The Telegram bot's /invite command hands the visitor a fresh code with a one-click magic link — no operator in the loop. The pricing page shows the remaining-slots number for the current round. An email channel is the fallback for people without Telegram.",
        items: [
    ...
    entries: [
      {
        ...
        body:
          "Перед запуском рекламы поставили собственный счётчик. Видим визиты, источники трафика, воронку от регистрации до оплаты и выручку по каждому источнику. Без сторонних сервисов и отслеживающих куки — данные о посещениях остаются только у нас. Внутренняя страница показывает всё в реальном времени за любую дату.",
        items: [
          "Счётчики посещений и источников трафика.",
          "Воронка: регистрация → оплата → выдача ключа.",
          "Выручка за день учитывается ровно один раз даже при повторных уведомлениях от платёжного сервиса.",
        ],
        status: "LIVE",
      },
      {
        date: "26 МАЯ 2026",
        label: "ПРИГЛАШЕНИЯ",
        title: "Автовыдача приглашений и публичный счётчик мест",
        body:
          "Доступ в бету теперь полностью самообслуживаемый. Telegram-бот по команде /invite сам выдаёт человеку код доступа со ссылкой для входа — оператор не нужен. На странице тарифа видно, сколько мест осталось в текущем раунде. Запасной канал для тех, кто без Telegram, — запрос приглашения по email со страницы тарифа.",
        items: [
          "Бот /invite: уникальный код и ссылка для входа за секунду.",
          "Публичный счётчик оставшихся мест в реальном времени.",
          "Запрос по email — запасной способ для тех, кто без Telegram.",
        ],
        status: "LIVE",
      },
      {
        ...
        body:
          "Регистрация и вход через Telegram-бот. Никаких паролей и писем с подтверждением — открыл ссылку, нажал «Подтвердить» в боте, оказался в кабинете. Каждый новый пользователь тратит один код приглашения; возвращающийся входит бесплатно. Вход по почте остаётся как запасной способ.",
        items: [
          "Безопасная серверная сессия после подтверждения в боте.",
          "Код приглашения списывается атомарно — повторно использовать нельзя.",
      ...
      },
      {
        date: "21 МАЯ 2026",
        label: "ОПЛАТА",
        title: "СБП и USDT подключены",
        body:
          "Подключили приём платежей: СБП через QR-код в банковском приложении и USDT-крипта. Оба способа ведут к одному и тому же оформлению оплаты. Подписка фиксируется в момент успешной оплаты — клиент видит активное состояние сразу. Ключ доступа пока выдаётся оператором; автоматизация выдачи следующая в очереди.",
        items: [
          "СБП QR — один тап в банковском приложении.",
          "USDT — обычная оплата криптовалютой.",
