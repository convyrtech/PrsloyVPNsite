    ...
    entries: [
      {
        ...
        body:
          "Перед запуском рекламы поставили собственный счётчик. Видим визиты, источники трафика, воронку от регистрации до оплаты и выручку по каждому источнику. Без сторонних сервисов и трекинг-куки — данные о посещениях остаются только у нас. Внутренняя страница показывает всё в реальном времени за любую дату.",
        items: [
      ...
      },
      {
        ...
        body:
          "Доступ в бету теперь полностью самообслуживаемый. Telegram-бот по команде /invite сам выдаёт человеку код доступа со ссылкой для входа — оператор не нужен. На странице тарифа видно, сколько мест осталось в текущем раунде. Альтернативный канал — запрос инвайта по email прямо со страницы тарифа.",
        items: [
      ...
      },
      {
        ...
        body:
          "Регистрация и вход через Telegram-бот. Никаких паролей и писем с подтверждением — открыл ссылку, нажал «Подтвердить» в боте, оказался в кабинете. Каждый новый юзер тратит один инвайт-код; возвращающийся входит бесплатно. Вход по почте остаётся как запасной способ.",
        items: [
