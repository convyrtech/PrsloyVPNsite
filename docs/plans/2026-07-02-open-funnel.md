...
- [ ] Настоящий отзыв (revoke) И смена ключа (rotate) — конечные точки на прокси-сервере (отдельный
      заход, бэкенд). Найдено основателем 2026-07-03: перевыпуск из админки сейчас невозможен одной
      кнопкой — /api/admin/issue продлевает тот же стабильный URL, ручной grant не гасит старый ключ;
      «второй ключ» на аккаунт не предусмотрен моделью. Задача в TODOS.md (P1).
